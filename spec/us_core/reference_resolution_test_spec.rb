RSpec.describe USCoreTestKit::ReferenceResolutionTest do
  ...
  describe '#unresolved_references' do
    ...
    before do
      stub_request(:get, "#{base_url}/#{patient_ref}")
      .to_return(status: 200, body: patient.to_json)

      stub_request(:get, "#{base_url}/#{practitioner_ref}")
        .to_return(status: 200, body: practitioner.to_json)

      stub_request(:get, "#{base_url}/#{organization_ref}")
        .to_return(status: 200, body: organization.to_json)

      stub_request(:get, "#{base_url}/#{encounter_ref}")
        .to_return(status: 200, body: encounter.to_json)
    end

    it 'returns empty when all references are resolved' do
      allow(test).to receive(:resource_is_valid?).and_return(true)

      resources = [diagnostic_report]
      result = test.unresolved_references(resources)
      expect(result.empty?).to be(true)
    end

    it 'returns empty when reference has display only' do
      allow(test).to receive(:resource_is_valid?).and_return(true)

      diagnostic_report.encounter = FHIR::Reference.new(display: 'this is a display text')
      resources = [diagnostic_report]
      result = test.unresolved_references(resources)
      expect(result.empty?).to be(true)
    end
